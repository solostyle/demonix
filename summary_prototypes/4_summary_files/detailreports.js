...
var ModulesManager = function () {
    ...
    function initializeModuleHandlers() {
		...
		// toggle the individual facet categories (modified by archana)
        facetsummaryWrapper.find('.facets li .headerText, .facets li .toggle').click(function () {
            $(this).find('img').toggle();
			$(this).parent().find('.facetNumberTitle').toggle();
...
var toggleTabs = function (tab) {
    $.each($('#SDTabs li'), function (i, val) {
        if ($(this).hasClass("selected") && (tab.attr('id') != $(this).attr('id'))) {
