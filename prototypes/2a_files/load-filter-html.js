	...
	jQuery('#facetsSupply .facets>li[class!="ignore"]>div:last-child').each(function populate() {
		...
		for (var i=0;i<currentFacet.length;i++) {
			...
			jQuery(this).find('ul')
				.append('<li></li>')
				.find('li:last')
				//.append('<div class="facetCheckbox"><input id="chkbx'+'_'+jQuery(this).attr('id')+'_'+facetName.replace(/\s/g,"")+'_'+'supply" type="checkbox" /></div>')
	...
	jQuery('#facetsDemand .facets>li[class!="ignore"]>div:last-child').each(function populate() {
		...
		for (var i=0;i<currentFacet.length;i++) {
			...
			jQuery(this).find('ul')
				.append('<li></li>')
				.find('li:last')
				//.append('<div class="facetCheckbox"><input id="chkbx'+'_'+jQuery(this).attr('id')+'_'+facetName.replace(/\s/g,"")+'_'+'demand" type="checkbox" /></div>')
				.append('<div class="filterCheckboxWrapper">'+facetName+'</div>')
